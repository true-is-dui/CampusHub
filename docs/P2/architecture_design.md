...
flowchart TB
    ...
    subgraph Backend["后端单体应用 Spring Boot"]
        ...
        Services["Service 层<br/>用户模块 | 实名审核模块 | 代取服务模块<br/>支付沙箱模块 | 文件模块 | 评价模块 | 通知模块"]
    ...
    Services --> FileStore
    Services --> Alipay
    Services -. "热点列表 / 临时状态 / 防重复操作" .-> Redis
```
...
2. 支付宝沙箱、完成凭证、确认完成、退款/结算等流程需要清晰事务边界，单体内同步调用更易控制一致性。
...
MVP 阶段采用同步内部调用作为主要协作方式。代取服务模块在发布有报酬服务时调用支付沙箱模块创建预付款记录；接单、上传完成凭证、确认完成、取消和超时取消均由代取服务模块维护状态流转；支付模块只维护支付、退款、结算记录，不反向理解完整代取业务。
...
flowchart TD
    ...
    Pickup -->|预付款 / 退款 / 结算| Payment[支付沙箱模块]
...
| 代取服务模块 | Must | 代取需求大厅、详情查看、发布代取、待支付/待接单/进行中/已完成/已取消状态流转、接单、完成凭证、确认完成、取消、超时取消、我的发布和我的接单记录 | FR-HALL-01~06, FR-PU-01~12 |
| 支付沙箱模块 | Must | 支付宝沙箱预付款、支付状态记录、退款、结算、防重复支付/退款的基础校验；不接入真实生产支付 | FR-PU-03, FR-PU-08, FR-PU-10, FR-WO-03 |
| 文件存储模块 | Must | 图片格式和大小校验、本地文件保存、文件元数据维护、文件内容读取；只提供后端内部文件读写能力，不对前端暴露独立文件接口；不保存上传者、业务用途或业务归属，业务模块保存 fileId 并先鉴权再读取 | FR-UM-02~04, FR-HALL-02, FR-PU-01, FR-PU-06~07 |
| 评价模块 | Should | 代取完成后双方互评、好评/中评/差评、差评理由、按发布方/接单方来源分别计算好评率、历史评价总览和详情查询 | FR-CR-01~04 |
| 站内通知模块 | Should | 保存认证结果、接单、完成凭证上传、确认完成、收到评价等通知记录，维护已读/未读状态 | FR-NT-01~02 |
...
| 私聊与聊天记录 | Could | 不作为当前 WebSocket/IM 架构决策；后续需要时再评估轮询、WebSocket 或第三方 IM |
...
|----------|------------|---------|------------------|------------------|
| 代取服务模块 / 评价模块 | 用户模块 | 同步内部调用 | 展示发布方、接单方或被评价方的公开摘要 | `getUserSummary(userId) -> UserSummaryDTO { userId, nickname, ratingSummary? }` |
| 用户模块 | 文件存储模块 | 同步内部调用 | 上传头像和实名认证材料；按用户资料访问权限读取头像 | `uploadImage(file) -> FileId`；`loadFile(fileId) -> FileContent` |
| 代取服务模块 | 文件存储模块 | 同步内部调用 | 保存取件凭证和完成凭证；接单成功后读取取件凭证；发布方确认完成前读取完成凭证 | `uploadImage(file) -> FileId`；`loadFile(fileId) -> FileContent` |
...
| 代取服务模块 | 支付沙箱模块 | 同步内部调用 | 发布有报酬代取服务时创建 3 分钟预付款入口；支付模块不保存具体业务对象 ID | `createPrepay(payerId, amount, expireAt) -> PrepayResultDTO { paymentId, payEntry, expireAt }` |
| 代取服务模块 | 支付沙箱模块 | 同步内部调用 | 查询支付状态；支付成功后代取服务进入待接单 | `queryPaymentStatus(paymentId) -> paymentStatus` |
| 代取服务模块 | 支付沙箱模块 | 同步内部调用 | 待支付取消、待支付超时或待接单取消时处理退款规则 | `refundPayment(paymentId) -> paymentStatus` |
| 代取服务模块 | 支付沙箱模块 | 同步内部调用 | 发布方确认完成后，有报酬服务结算给接单方 | `settlePayment(paymentId, receiverId) -> paymentStatus` |
| 评价模块 | 代取服务模块 | 同步内部调用 | 评价提交前校验服务已完成且评价人属于发布方或接单方 | `queryPickupEvaluationContext(pickupId) -> PickupEvaluationContextDTO { publisherId, acceptorId, pickupStatus }` |
| 评价模块 | 代取服务模块 | 同步内部调用 | 评价创建成功后，由代取服务记录该服务关联的评价 ID | `recordPickupEvaluation(pickupId, evaluationId) -> void` |
...
| 状态 | 进入条件 | 可执行操作 | 下一状态 |
|------|---------|-----------|---------|
| 待支付 | 有报酬代取服务提交成功 | 发布方继续支付或取消；系统检查 3 分钟超时 | 支付成功进入待接单；取消或超时进入已取消 |
| 待接单 | 无报酬发布成功，或有报酬支付成功 | 其他认证用户接单；发布方取消；系统检查接单截止时间 | 接单进入进行中；取消或截止超时进入已取消 |
| 进行中 | 认证用户接单成功 | 接单方上传完成凭证；发布方查看凭证并确认完成 | 确认完成进入已完成 |
| 已完成 | 发布方确认完成 | 有报酬服务触发沙箱结算；开放评价入口 | 终态 |
| 已取消 | 发布方取消、待支付超时或接单截止超时 | 有报酬且已预付款时触发退款 | 终态 |
...
| NFR-MA-02：关键操作日志 | 登录、实名认证审核、发布代取、支付、接单、上传完成凭证、确认完成、取消、退款、结算、评价和通知等关键操作记录日志 | MVP 可采用数据库审计表或应用日志文件，优先保证可追溯 |
...
| 主数据库 | MySQL 8.0 | 用户、实名审核、代取服务、支付记录、评价、通知均为结构化数据 |
| 缓存 | Redis 7.0（可选） | 可用于热点代取列表、支付防重复操作、登录限流等后续优化；不作为 MVP 必选 |
| 文件存储 | 本地文件存储 | 头像、认证材料、取件凭证、完成凭证保存到服务器本地目录，MySQL 仅保存文件路径、MIME 类型、大小等本地文件元数据 |
| 支付 | 支付宝沙箱 | 只验证有报酬代取服务的预付款、退款和结算流程，不发生真实资金流转 |
...
- 站内通知是 Should，可先用数据库通知记录和前端轮询/手动刷新实现。
...
| 独立 IM / WebSocket 长连接 | 私聊为 Could，不是 MVP 必选；站内通知可先用数据库记录实现 |
...
| 真实线上支付 | P1 明确 Won't，本次仅使用支付宝沙箱验证流程 |
