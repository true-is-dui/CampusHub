...
MVP 阶段采用同步内部调用作为主要协作方式。代取服务模块在发布有报酬服务时调用支付沙箱模块创建预付款记录；接单、上传完成凭证、确认完成、取消和超时取消均由代取服务模块维护状态流转。支付沙箱模块不负责维护完整代取服务业务对象，也不参与代取服务的业务状态流转。支付记录可以保存业务类型、业务追踪号、外部支付订单号等追踪字段，这些字段仅用于幂等校验、日志追踪、异常排查和支付宝沙箱回调匹配，不作为支付模块理解或修改代取业务状态的依据。
...
| 支付沙箱模块 | Must | 支付宝沙箱预付款、支付状态记录、退款、结算、防重复支付/退款的基础校验；支付记录只保存业务类型、业务追踪号、外部支付订单号等追踪字段，不维护代取业务状态，不接入真实生产支付 | FR-PU-03, FR-PU-08, FR-PU-10, FR-WO-03 |
...
| 代取服务模块 | 支付沙箱模块 | 同步内部调用 | 发布有报酬代取服务时创建 3 分钟预付款入口；支付记录只保存业务追踪字段，不维护代取业务状态 | `createPrepay(payerId, amount, expireAt, trackingInfo) -> PrepayResultDTO { paymentId, payEntry, expireAt }` |
| 代取服务模块 | 支付沙箱模块 | 同步内部调用 | 查询支付状态；代取服务模块根据支付结果决定服务是否进入待接单 | `queryPaymentStatus(paymentId) -> paymentStatus` |
| 代取服务模块 | 支付沙箱模块 | 同步内部调用 | 待接单取消或接单截止超时且已完成预付款时处理退款；代取状态仍由代取服务模块维护 | `refundPayment(paymentId) -> paymentStatus` |
| 代取服务模块 | 支付沙箱模块 | 同步内部调用 | 发布方确认完成后，有报酬服务结算给接单方；结算结果不由支付模块直接改写代取状态 | `settlePayment(paymentId, receiverId) -> paymentStatus` |
...
MVP 阶段代取服务超时取消采用定时任务扫描 + 业务访问时惰性校验结合实现。后端定时任务每 1 分钟扫描待支付和待接单服务：待支付服务超过 3 分钟仍未支付成功时，状态更新为已取消；待接单服务超过接单截止时间仍无人接单时，状态更新为已取消。用户查看列表、查看详情、继续支付、接单或取消服务时，后端也必须先校验该服务是否已经超时；如果已超时，先更新状态为已取消，再返回最新状态。

| 服务状态 | 超时取消规则 |
|---------|-------------|
| 待支付 | 超过 3 分钟仍未支付成功，改为已取消，不涉及退款 |
| 待接单 | 超过接单截止时间仍无人接单，改为已取消；如为有报酬且已完成预付款，则由代取服务模块触发支付沙箱模块退款 |
