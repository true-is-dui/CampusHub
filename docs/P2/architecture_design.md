...
flowchart TB
    ...
    subgraph ApiLayer["表示层 API Layer"]
        Nginx["Nginx 反向代理<br/>RESTful API 统一入口"]
    end

    subgraph BusinessLayer["业务层 Business Layer"]
        Modules["用户模块 | 代取服务模块 | 支付模块 | 评价模块 | IM模块 | 管理模块<br/>失物招领模块 | 二手交易模块 | 文件存储模块"]
    end
    ...
    Browser -->|"HTTP/JSON<br/>WebSocket"| Nginx
    Nginx --> Modules
    Modules --> MySQL
    Modules -. "缓存 / 临时状态 / 在线状态" .-> Redis
```
...
| **需求明确度** | P1阶段已完成完整SRS，P4回溯后MVP范围收敛为代取服务、失物招领和二手交易，模块边界更清晰，分层架构可良好映射 |
...
3. 分层架构通过良好的模块划分（见第四节），可以在用户量增长后按需将热点模块（如代取服务模块、IM模块）独立拆分
...
事件驱动属于隐式调用风格，模块通过发布和订阅事件解耦，扩展性更强，例如代取服务完成后可以扩展评价、通知、统计等后续处理。但该方式会引入事件模型、监听关系、调试链路和一致性处理；若引入RabbitMQ、Kafka等消息队列，还会增加部署、监控和运维成本。结合4人团队、10周MVP和团队技术储备，当前阶段不把事件驱动作为主要模块协作方式。
...
基于P1最新需求规格说明书，将系统划分为以下9个逻辑模块。这里的模块表示单体后端内部的业务/支撑能力边界，用于说明职责、依赖关系和模块间接口。P4回溯评审后，服务板块仅保留代取服务，互助板块仅保留失物招领和二手交易；找搭子、咨询问答、代购/代办不进入MVP模块划分。
...
flowchart TD
    ...
    User -->|头像 / 认证材料| File
    User -->|认证审核记录| Admin
    Pickup -->|预付款 / 转账 / 退款| Payment
    Rating -->|评价资格判断| Pickup
    Pickup -->|完成凭证图片| File
    Pickup -->|审核 / 异常申诉记录| Admin
...
认证边界：用户模块处理用户名密码注册、用户名密码登录、实名认证资料提交、认证状态、账号状态和个人资料维护；MVP阶段不接入运营商短信验证码，图形验证码作为后续安全增强。登录成功后签发JWT。公开浏览接口按板块进入对应核心业务模块；受保护接口由JWT拦截器校验登录状态和账号状态，并将`currentUserId`、`role`、`authStatus`、`accountStatus`放入请求上下文，各业务模块从上下文获取当前用户身份后执行业务规则。后台管理接口需校验`role=ADMIN`，管理操作使用请求上下文中的管理员身份记录处理人。

需求大厅作为前端页面组织方式实现：前端在同一大厅页面内按“服务”和“互助”两个一级入口切换。服务入口调用代取服务模块公开列表接口，支持校区、报酬类型、关键词和发布时间倒序；互助入口分别调用失物招领和二手交易模块公开列表接口，并支持互助类型、校区、关键词和发布时间倒序。MVP阶段不提供跨板块综合大厅或跨板块搜索。

### 4.2 模块详细说明

| 模块 | 职责 | 关联需求 |
|------|------|---------|
| **用户模块** | 用户名密码注册、用户名密码登录、认证状态维护、账号状态维护、个人资料、用户中心聚合查询 | FR-UM-01~03, FR-UM-05, FR-AD-01 |
| **代取服务模块** | 代取服务发布（含取件地点、送达地点、物品描述、取件凭证、报酬类型、校区、接单截止时间）、接单、状态流转、完成凭证、确认完成、取消/异常申诉协作、触发报酬结算；不支持代购、代办或需要接单方垫付本金的服务 | FR-ER-01~05 |
| **支付模块** | 支付宝沙箱预付款、待支付锁单协作、转账、退款、交易记录存储与查询、防重复支付/退款 | FR-UM-04, FR-ER-01, FR-ER-03 |
...
| **管理模块** | 实名认证审核、账号封禁、代取服务和互助内容审核、内容举报、用户举报、评价申诉、代取服务异常申诉处理、聊天记录查看、处理结果触发代取服务状态调整及后续支付协作 | FR-UM-05, FR-AD-01~03, FR-CR-04~05, FR-TM-04 |
| **文件存储模块** | 图片上传、JPG/PNG格式校验、5MB大小校验、本地保存、文件标识与本地路径映射维护、文件内容读取 | FR-UM-03, FR-UM-05, FR-ER-04, FR-SH-01, FR-IM-04 |
...
| 代取服务模块 | 支付模块 | 业务流程协作 | 发布有报酬代取服务时先生成发布者可见的待支付服务记录，并创建支付宝沙箱预付款入口；待支付服务不进入需求大厅和管理员内容审核队列，发布者可继续支付或手动取消，超过支付有效期未支付时由系统自动取消；代取服务模块使用`paymentId`跟踪该服务对应的支付记录；支付成功后预付款暂存平台沙箱账户，服务进入审核中状态并创建内容审核记录；服务取消/审核驳回时退款；服务确认完成时结算报酬 | `createPrepay(payerId, amount) -> PrepayResultDTO { paymentId, payEntry, expireAt }`；`queryPaymentStatus(paymentId) -> paymentStatus`；`refundPayment(paymentId) -> paymentStatus`；`settlePayment(paymentId, receiverId) -> paymentStatus` |
...
| 评价模块 | 代取服务模块 | 同步查询 | 查询代取服务评价上下文，用于评价入口展示和评价提交时的资格校验 | `queryPickupEvaluationContext(pickupId) -> PickupEvaluationContextDTO { publisherId, acceptorId, pickupStatus }` |
| 用户模块 / 代取服务模块 / 二手交易模块 / IM模块 | 文件存储模块 | 同步内部调用 | 上传头像、认证材料、完成凭证、商品图和私聊图片；前端通过对应业务模块获取文件，业务模块完成权限校验后再调用文件模块读取内容 | `uploadImage(file) -> FileId`；`loadFile(fileId) -> FileContent { bytes, mimeType }` |
...
| 用户模块 | 管理模块 | 用户举报记录协作 | 用户举报可疑账号后，用户模块向管理模块创建用户举报记录；举报原因由管理模块记录 | `createUserReport(targetUserId, reporterId, reason) -> managementRecordId` |
| 代取服务模块 / 失物招领模块 / 二手交易模块 | 管理模块 | 内容审核记录协作 | 业务模块创建审核中内容后，向管理模块创建内容审核记录；管理模块只记录处理对象标识，不复制业务内容 | `createContentReview(targetType, targetId) -> managementRecordId` |
| 管理模块 | 代取服务模块 / 失物招领模块 / 二手交易模块 | 内容审核结果协作 | 管理员处理内容审核记录后，管理模块调用对应业务模块更新内容状态 | `updateContentReviewStatus(targetId, reviewResult) -> contentStatus` |
| 失物招领模块 / 二手交易模块 | 管理模块 | 内容举报记录协作 | 用户对已发布互助内容提交举报后，业务模块向管理模块创建内容举报记录；举报原因由管理模块记录 | `createContentReport(targetType, targetId, reason) -> managementRecordId` |
| 评价模块 | 管理模块 | 评价申诉记录协作 | 用户对评价提交举报/申诉后，评价模块向管理模块创建评价申诉记录；申诉原因由管理模块记录 | `createEvaluationAppeal(evaluationId, reason) -> managementRecordId` |
| 代取服务模块 | 管理模块 | 代取服务异常申诉记录协作 | 用户提交代取服务异常申诉后，代取服务模块向管理模块创建异常申诉记录；申诉原因由管理模块记录 | `createPickupAppeal(pickupId, reason) -> managementRecordId` |
| 管理模块 | 失物招领模块 / 二手交易模块 | 举报处理结果协作 | 管理员处理互助内容举报后，管理模块调用对应业务模块更新目标状态 | `updateReportedContentStatus(targetId, handleResult) -> contentStatus` |
...
| 管理模块 | 代取服务模块 / IM模块 | 代取服务异常申诉处理 | 管理员处理代取服务异常申诉时读取服务信息、完成凭证和聊天记录；处理完成后交给代取服务模块更新服务状态并触发退款或结算 | `handlePickupAppealResult(pickupId, handleResult) -> AppealHandleResultDTO { pickupStatus, paymentStatus }`；`queryAppealChatRecords(pickupId) -> 聊天记录` |
...
| `NFR-PF-02` 板块切换、校区筛选、关键词搜索不超过2秒 | 代取服务、失物招领、二手交易模块分别负责本板块的筛选、搜索和排序，避免跨模块聚合查询；前端使用Vue Router和组件状态保留页面上下文，切换板块时只请求目标板块数据 | 关键词搜索以MySQL条件查询为基础，复杂全文检索不作为MVP架构依赖 |
...
| `NFR-MA-02` 关键操作日志保存不少于30天 | 用户、代取服务、支付、评价、管理和IM等模块在登录、发布、接单、支付、退款、评价、审核、封禁、申诉处理等关键业务点记录操作日志；管理操作使用请求上下文中的管理员身份记录处理人 | 日志可采用数据库审计表或应用日志文件，MVP阶段优先保证关键操作可追溯 |
...
**为什么选MyBatis Plus？**
- 项目主数据库为MySQL，业务数据以用户、代取服务、互助发布内容、评价、聊天记录等结构化表为主，MyBatis Plus适合快速完成常规CRUD、分页和条件查询
...
flowchart TB
    ...
    Nginx -->|"REST API / WebSocket 代理"| Spring
    Spring --> MySQL
    Spring -. "缓存 / 临时状态 / 在线状态" .-> Redis
```
