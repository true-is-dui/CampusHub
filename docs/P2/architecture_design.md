...
**AI回答摘要：**
AI扮演架构师，给出了"分层架构 + 事件驱动补充"的混合方案。建议核心业务流程（发布-接单-完成）使用分层架构，通知推送和积分变动使用事件驱动。技术栈推荐Spring Boot + MySQL + Redis + RabbitMQ + WebSocket。
...
- 事件驱动建议有一定价值（积分变动确实适合事件驱动），但引入MQ增加了运维复杂度
...
| 模块 | 英文名 | 职责 | 关联需求 |
|------|--------|------|---------|
| **用户服务** | UserService | 注册、登录、学号认证、个人资料、积分管理、签到 | FR-UM-01~06 |
| **任务服务** | TaskService | 发布跑腿任务（含报酬设置、校区、有效期） | FR-ER-01 |
| **订单服务** | OrderService | 接单、状态流转、确认完成、凭证上传、积分转移 | FR-ER-02~04 |
...
| **咨询问答服务** | QAService | 问题发布、回答、最佳答案采纳、积分奖励 | FR-QA-01~04 |
...
| **数据库（缓存）** | Redis 7.0 | 会话管理、积分排行榜、需求列表高频缓存、IM在线状态 |
...
- 通知推送和积分变动用Spring Event + 异步方法即可，引入MQ增加运维成本
