...
| **后置条件** | 1. 无报酬服务提交成功后，状态为“待接单”<br>2. 有报酬服务提交成功后，系统从发布方积分余额中扣减报酬积分，扣减成功后状态变为“待接单”<br>3. 有报酬服务发布时积分余额不足，发布失败并提示积分不足 |
...
| 4 | 用户确认发布 |
| 5 | 若为有报酬服务，系统校验发布方积分余额并扣减对应积分 |
| 6 | 系统将服务状态设为待接单（有报酬在扣减积分后进入、无报酬直接进入） |
| 7 | 待接单服务进入代取需求大厅展示 |
...
| **后置条件** | 1. 接单成功后服务进入进行中状态<br>2. 接单方上传完成凭证后，发布方可确认完成<br>3. 发布方确认完成后服务进入已完成状态<br>4. 有报酬服务将报酬积分从发布方账户转入接单方账户 |
...
| 9 | 发布方查看完成凭证并确认完成 |
| 10 | 系统将服务状态更新为已完成 |
| 11 | 有报酬服务将报酬积分从发布方转入接单方；无报酬服务不进行积分处理 |
| 12 | 系统开放双方互评入口 |
...
| 3 | “我的发布”提供总览，并支持按状态查看记录：待接单、进行中、已完成、已取消 |
...
| **参与者** | 普通用户 |
| **前置条件** | 用户已登录系统 |
| **后置条件** | 1. 签到成功后系统向用户赠送 5 积分<br>2. 同一用户当日重复签到时不再赠送积分 |
