...
本文是 P3 阶段详细设计整合版，用于汇总说明当前系统的详细设计范围、三份独立设计文档之间的一致性、核心业务闭环和 AI 辅助设计记录。
...
当前系统范围已经收敛为“校园可信代取服务闭环”，承接 P1/P2/P3 最新设计。系统围绕真实学生身份、代取需求发布、支付、接单、履约、确认、评价和通知形成最小可实现闭环。
...
| `PickupRequest` | 代取业务主聚合，承载发布、支付、接单、完成、取消等主状态 | 关联发布方、接单方、取件凭证、完成凭证和主支付记录 |
| `PaymentRecord` | 保存支付宝沙箱预付款、关闭、退款、结算等支付状态 | 有偿代取由 `PickupRequest.paymentId` 指向主支付记录 |
| `Evaluation` | 支持代取完成后的双方评价和好评率统计 | 通过 `businessType=PICKUP_REQUEST`、`businessId` 定位代取请求 |
| `NotificationRecord` | 保存站内通知、已读状态和业务回溯信息 | 由认证、接单、完成、支付、评价等业务事件触发 |

分层协作摘要：

| 层次 | 主要对象 | 说明 |
|---|---|---|
| Controller | Auth、Users、Admin、PickupRequests、Payments、Evaluations、Notifications 控制器 | 接收 HTTP 请求、解析参数、返回统一响应，不直接处理业务状态流转 |
| Service | `AuthService`、`UserService`、`VerificationReviewService`、`FileStorageService`、`PickupService`、`PaymentService`、`EvaluationService`、`NotificationService` | 负责权限校验、状态校验、跨模块协作和事务边界 |
| Repository | 用户、审核、文件、代取、支付、评价、通知仓储 | 面向核心表进行持久化，不承载业务流程判断 |
| Domain / Entity | `User`、`VerificationReview`、`StoredFile`、`PickupRequest`、`PaymentRecord`、`Evaluation`、`NotificationRecord` | 表达领域状态和必要行为 |

`PickupRequest` 是代取业务主聚合对象，统一承载 `WAITING_PAYMENT`、`WAITING_ACCEPT`、`IN_PROGRESS`、`COMPLETED`、`CANCELLED` 等主业务状态。文件、支付、评价、通知分别作为独立模块与代取业务协作，避免在代取对象中堆积所有实现细节。
...
| 容易将代取主流程拆成 `Task + Order` 双主对象 | 双主对象会造成发布状态、支付状态、接单状态和履约状态分裂 | 使用 `PickupRequest` 统一承载主业务状态，支付作为独立记录被引用 |
| 容易把文件、支付、评价的追踪字段理解成双向强外键 | 追踪字段的目标是溯源、幂等和扩展，不应反向驱动业务状态 | `StoredFile.businessType/businessId`、`PaymentRecord.businessType/businessTraceNo`、`Evaluation.businessType/businessId` 均明确边界 |
| Service 职责边界容易混乱 | 一个大服务同时处理认证、文件、代取、支付、评价、通知会破坏单一职责 | 拆分为认证、用户、实名审核、文件、代取、支付、评价、通知服务 |

SOLID 修正后的结果：

- 单一职责：用户、文件、代取、支付、评价、通知分开建模。
- 开闭原则：文件上传处理器、支付网关适配器支持后续替换和扩展。
- 里氏替换：不再强行使用跨业务 `Task` 继承层级。
- 接口隔离：Controller / Service 按业务模块拆分。
- 依赖倒转：业务层依赖 `PaymentGateway` 抽象，而不是直接依赖支付宝 SDK 细节。
...
| 适配器模式 | 支付宝沙箱预付款、回调验签、关闭、退款、结算 | `PaymentService` 依赖 `PaymentGateway` 抽象，支付宝 SDK 细节由 `AlipaySandboxPaymentGateway` 封装 | 业务服务会直接耦合支付宝参数、返回码和异常形式，后续替换支付渠道或模拟测试困难 |

当前类设计没有额外引入状态模式。代取状态数量有限，状态流转由 `PickupService` 方法和 `PickupRequest` 行为共同约束即可。

## 六、API 设计摘要

`api_design.yaml` 使用 OpenAPI 3.0.3 描述接口。接口默认使用 JWT Bearer 认证，注册、登录、公开用户主页、头像、代取大厅、代取详情和支付宝回调等接口按业务需要开放匿名访问。成功响应统一使用 `ApiResponse`，错误响应统一使用 `ErrorResponse`，当前错误码包括参数错误、认证失败、权限不足、资源不存在、业务冲突、系统异常和第三方服务异常。
...
| Pickup Requests | `GET /pickup-requests`、`POST /pickup-requests`、`GET /pickup-requests/{pickupId}`、`GET /pickup-requests/{pickupId}/credential`、`GET /pickup-requests/{pickupId}/payment-entry`、`POST /pickup-requests/{pickupId}/accept`、`POST /pickup-requests/{pickupId}/completion-proof`、`GET /pickup-requests/{pickupId}/completion-proof`、`POST /pickup-requests/{pickupId}/completion-confirmation`、`POST /pickup-requests/{pickupId}/cancel`、`GET /users/me/pickup-requests` | 覆盖发布、列表、详情、接单、完成凭证、确认完成、取消、我的发布、我的接单 |
| Payments | `GET /pickup-requests/{pickupId}/payment-entry`、`GET /payments/{paymentId}`、`POST /payments/alipay/notify`、`GET /users/me/transactions` | 有偿代取发布时创建预付款；支付成功后代取进入 `WAITING_ACCEPT`；支付宝回调用于支付幂等处理 |
...
- 发布有偿代取后，`PickupRequest.status=WAITING_PAYMENT`，`PaymentRecord.status=WAITING_PAY`。
- 支付宝沙箱支付成功并通过回调处理后，代取进入 `WAITING_ACCEPT`，可在大厅展示并被接单。
- 待支付超时或发布方主动取消待支付服务时，代取进入 `CANCELLED`，支付记录进入 `CLOSED`。
- 已支付但仍待接单的服务取消或接单截止超时时，代取进入 `CANCELLED`，支付记录进入 `REFUNDED`。
- 发布方确认完成后，代取进入 `COMPLETED`，有偿服务支付记录进入 `SETTLED`。
...
| `pickup_requests` | 代取请求主表 | 替代旧版双表模型，统一维护发布、支付后可接单、接单、完成、取消和超时状态 |
| `payment_records` | 支付记录 | 支撑支付幂等、支付状态、支付宝回调、关闭、退款和结算追踪 |
...
- `pickup_requests.status` 取值为 `WAITING_PAYMENT`、`WAITING_ACCEPT`、`IN_PROGRESS`、`COMPLETED`、`CANCELLED`。
- `pickup_requests.cancel_reason` 取值为 `USER_CANCELLED`、`PAYMENT_EXPIRED`、`ACCEPT_DEADLINE_EXPIRED`、`SYSTEM_CANCELLED`。
- `payment_records.status` 取值为 `WAITING_PAY`、`PAID`、`CLOSED`、`REFUNDED`、`SETTLED`。
- `payment_records.business_type` 和 `business_trace_no` 用于支付追踪、幂等、支付宝沙箱回调定位和异常排查，不作为反向业务外键。
...
- 当前数据库基本满足第三范式；用户、审核、文件、代取、支付、评价、通知职责拆分清晰。
- 少数快照、追踪、扩展字段有明确业务语义，不属于无意义冗余。
- 索引覆盖登录、学号唯一、认证审核、代取大厅、我的发布、我的接单、支付回调、待支付超时扫描、接单截止超时扫描、通知未读、评价统计等高频路径。
- 当前 MVP 数据量下暂不需要分表、缓存统计表或独立消息队列。

不保留的旧版数据库内容包括失物招领、搭子招募、二手交易、问答评论、私聊消息、订单、封禁申诉、复杂审计日志等旧范围表，也不恢复旧版支付记录结构。
...
| 流程节点 | 主要 API | 核心类/服务 | 数据库变化 |
|---|---|---|---|
| 发布有偿代取 | `POST /pickup-requests` | `PickupService`、`PaymentService`、`PaymentGateway` | 新增 `pickup_requests.status=WAITING_PAYMENT`；新增 `payment_records.status=WAITING_PAY`；代取写入 `payment_id` |
| 获取支付入口 | `GET /pickup-requests/{pickupId}/payment-entry` | `PickupService`、`PaymentService` | 状态不变，仅返回仍有效的支付入口 |
| 支付宝回调 | `POST /payments/alipay/notify` | `PaymentService`、`PickupService` | 支付变为 `PAID`，代取变为 `WAITING_ACCEPT` |
| 查询支付状态 | `GET /payments/{paymentId}` | `PaymentService` | 读取 `payment_records` 当前状态 |
...
| 流程节点 | 主要 API | 核心类/服务 | 数据库变化 |
|---|---|---|---|
| 发布方确认 | `POST /pickup-requests/{pickupId}/completion-confirmation` | `PickupService`、`PaymentService`、`NotificationService` | 代取状态变为 `COMPLETED`，写入 `completed_at` |
| 有偿服务结算 | 同上 | `PaymentService`、`PaymentGateway` | 支付状态变为 `SETTLED`，写入接单方收款信息 |

### 8.7 主动取消与超时取消

| 流程节点 | 主要 API | 核心类/服务 | 数据库变化 |
|---|---|---|---|
| 发布方主动取消待支付 | `POST /pickup-requests/{pickupId}/cancel` | `PickupService`、`PaymentService` | 代取变为 `CANCELLED`，`cancel_reason=USER_CANCELLED`，支付变为 `CLOSED` |
| 发布方主动取消待接单 | `POST /pickup-requests/{pickupId}/cancel` | `PickupService`、`PaymentService` | 代取变为 `CANCELLED`；已支付有偿服务支付变为 `REFUNDED` |
| 支付超时取消 | 后端定时扫描 | `PickupService`、`PaymentService` | 代取变为 `CANCELLED`，`cancel_reason=PAYMENT_EXPIRED`，支付变为 `CLOSED` |
| 接单截止超时取消 | 后端定时扫描或业务访问前校验 | `PickupService`、`PaymentService` | 代取变为 `CANCELLED`，`cancel_reason=ACCEPT_DEADLINE_EXPIRED`；已支付则退款 |
...
| 流程节点 | 主要 API | 核心类/服务 | 数据库变化 |
|---|---|---|---|
| 业务事件生成通知 | 认证审核、接单、完成凭证、确认完成、支付、评价等业务接口 | `NotificationService` | 新增 `notifications` |
...
| 用户类职责 | 初稿可能让 `User` 直接处理支付和好评率 | 违反单一职责 | `User` 仅处理身份、资料和认证状态；评价统计由 `EvaluationService` 完成 |
| 文件关系 | 初稿可能把文件溯源字段画成业务强关系 | 文件表不应承担权限判断 | 文件只保存元数据和上传溯源，业务对象通过文件 ID 引用 |
| 服务边界 | 初稿可能设计大而全平台服务 | 接口过胖且难以测试 | 拆分为认证、用户、审核、文件、代取、支付、评价、通知服务 |
...
| 取消原因 | 初稿只给出 `CANCELLED` 状态 | 无法区分主动取消、支付超时和接单截止超时 | 补充 `PickupCancelReason`：`USER_CANCELLED`、`PAYMENT_EXPIRED`、`ACCEPT_DEADLINE_EXPIRED`、`SYSTEM_CANCELLED` |
| 支付状态 | 初稿只描述业务状态，缺少支付内部状态 | 有偿代取需要处理待支付、关闭、退款、结算 | 补充 `PaymentStatus`：`WAITING_PAY`、`PAID`、`CLOSED`、`REFUNDED`、`SETTLED` |
...
| 审查对象 | AI 审查发现 | 人工判断 | 最终处理 |
|---|---|---|---|
| 第三范式 | 表职责基本清晰，但快照和追踪字段容易被误判为冗余 | 快照、追踪、扩展字段有明确业务语义 | 保留实名审核快照、文件溯源、支付追踪、评价业务定位字段 |
| 索引合理性 | 高频查询路径需要明确索引支撑 | 登录、认证审核、大厅、我的记录、支付回调、超时扫描、通知未读、评价统计都是核心路径 | 在对应表设计唯一约束和普通索引 |
| 潜在性能瓶颈 | 初稿可能提前引入分表、缓存统计表、消息队列 | 当前课程 MVP 数据量不需要过度设计 | 暂不新增分表、缓存统计表或独立消息队列 |
| 支付关系 | 初稿可能让支付表反向保存具体代取外键并驱动业务状态 | 支付模块不应拥有代取业务状态 | 仅由 `pickup_requests.payment_id` 指向主支付记录；支付追踪字段不反向约束代取 |
...
| 日志和归档 | 初稿可能新增日志表、支付流水表、通知归档表 | 当前 MVP 不需要新增复杂辅助表 | 不新增日志表、支付流水表、通知归档表；关键异常通过应用日志记录 |
...
- `PickupRequest`、`PaymentRecord`、`StoredFile`、`Evaluation`、`NotificationRecord` 等核心对象在类、API、数据库中保持一致。
- 代取状态统一为 `WAITING_PAYMENT`、`WAITING_ACCEPT`、`IN_PROGRESS`、`COMPLETED`、`CANCELLED`。
- 支付状态统一为 `WAITING_PAY`、`PAID`、`CLOSED`、`REFUNDED`、`SETTLED`。
- 文件引用、支付引用、评价引用和通知回溯关系已基本对齐。
- 文件模块、支付模块、评价模块、通知模块均作为独立模块与代取主流程协作，不再形成旧版大而全模型。
