      ...
      <el-card v-if="ratingSummary" class="rating-card" shadow="never">
        ...
        <div class="rating-summary">
          <div v-if="ratingSummary.publisherRoleSummary" class="rating-role">
            ...
            <span class="positive-rate">好评率 {{ ratingSummary.publisherRoleSummary.positiveRate?.toFixed(1) || '0.0' }}%</span>
          </div>
          <div v-if="ratingSummary.acceptorRoleSummary" class="rating-role">
            ...
            <span class="positive-rate">好评率 {{ ratingSummary.acceptorRoleSummary.positiveRate?.toFixed(1) || '0.0' }}%</span>
          </div>
        </div>
      </el-card>
...
import { getUserProfile, getUserAvatar, getUserRatingSummary, getUserEvaluations } from '@/api/user'
...
const avatarSrc = computed(() => {
  if (user.value?.id || route.params.id) {
    return getUserAvatar(route.params.id)
  }
  return ''
})
...
async function loadUser() {
  ...
  try {
    ...
    const [profileRes, ratingRes] = await Promise.all([
      ...
      getUserRatingSummary(userId).catch(() => null)
    ])
    user.value = profileRes.data
    ratingSummary.value = ratingRes?.data || null
  } catch {
...
async function loadEvaluations() {
  ...
  try {
    ...
    evalTotal.value = res.data?.total || 0
  } catch {
    // error handled by interceptor
  } finally {
